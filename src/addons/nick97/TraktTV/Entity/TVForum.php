...
class TVForum extends Entity
{
	...
	public function rebuildRating($autoSave = false)
	{
		$rating = $this->db()->fetchRow("
			SELECT COUNT(*) AS total,
				SUM(rating) AS sum
			FROM nick97_trakt_tv_ratings_node
			WHERE node_id = ?
		", [$this->node_id]);
	...
	{
		$db = $this->db();
		$db->delete('nick97_trakt_tv_ratings_node', 'node_id = ?', $this->node_id);

		if ($this->tv_image) {
			/** @var \nick97\TraktTV\Service\TVForum\Image $imageService */
			$imageService = $this->app()->service('nick97\TraktTV:TVForum\Image', $this);
	...
	{
		$structure->table = 'nick97_trakt_tv_forum';
		$structure->shortName = 'nick97\TraktTV:TVForum';
		...
		$structure->relations = [
			...
			'Ratings' => [
				'entity' => 'nick97\TraktTV:RatingNode',
				'type' => self::TO_MANY,
				'conditions' => 'node_id',
				'key' => 'user_id',
				'primary' => false
