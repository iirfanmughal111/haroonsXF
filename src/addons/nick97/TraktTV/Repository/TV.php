...
class TV extends \XF\Mvc\Entity\Repository
{
	...
	public function insertOrUpdateShowCredits($tvId, $season = 0, $episode = 0, array $rawCredits = [])
	{
		...
		if ($castData) {
			$db->insertBulk('nick97_trakt_tv_cast', $castData, true);
		}
		...
		if ($crewData) {
			$db->insertBulk('nick97_trakt_tv_crew', $crewData, true);
		}
	}
	...
	protected function getCastDataForInsert($cast, $tvId, $season = 0, $episode = 0)
	{
		...
		$entity = $this->em->create('nick97\TraktTV:Cast');
	...
	{
		/** @var \nick97\TraktTV\Entity\Crew $entity */
		$entity = $this->em->create('nick97\TraktTV:Crew');
	...
	{
		/** @var \nick97\TraktTV\Entity\Video $entity */
		$entity = $this->em->create('nick97\TraktTV:Video');
