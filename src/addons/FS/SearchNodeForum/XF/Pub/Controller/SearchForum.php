...
class SearchForum extends XFCP_SearchForum
{
	...
	public function actionSearch(ParameterBag $params){
		...
		if (!$threadIds){
			return $this->rerouteController('XF:SearchForum', 'Index', $params);
			
		}

		$finder = $this->getSearchForumRepo()->getThreadByApplyFilter( explode (",", $threadIds['results']), $params);
		$total  = $finder->total();
    ...
    public function actionView(ParameterBag $params)
	{
		...
		// var_dump($filters);
		$session = \XF::session();
		if (count($filters) || $this->filter('apply', 'bool'))
		{	
