...
class Listener
{
	...
	public static function forumEntityStructure(\XF\Mvc\Entity\Manager $em, \XF\Mvc\Entity\Structure &$structure)
	{
		$structure->columns['default_sort_order'] = ['type' => Entity::STR, 'default' => 'last_post_date',
			'allowedValues' => ['title', 'post_date', 'reply_count', 'view_count', 'last_post_date', 'Movie.tmdb_rating']
		];
	}
	...
	{
		$structure->columns['snog_movies_thread_count'] = ['type' => Entity::UINT, 'default' => 0];
	}
	...
	public static function criteriaUser(string $rule, array $data, \XF\Entity\User $user, bool &$returnValue)
	{
		...
		{
			/** @var \Snog\Movies\XF\Entity\User $user */
			$returnValue = $user->snog_movies_thread_count && $user->snog_movies_thread_count >= $data['movies'];
		}
	}
	...
	{
		$structure->columns['snog_movies_tmdb_watch_region'] = ['type' => Entity::STR, 'default' => 'US', 'maxLength' => 2];
	}
	...
	{
		if ($arguments['group'] instanceof \XF\Entity\OptionGroup && $arguments['group']->group_id == 'snogMovies')
		{
			$template = 'snog_movies_option_macros';
