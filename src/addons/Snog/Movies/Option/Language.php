...
class Language
{
	public static function verifyOption(&$value, \XF\Entity\Option $option)
	{
		$verifyCodes = [
			...
			'tn', 'to', 'tr', 'ts', 'tt', 'tw', 'ty', 'ug', 'ur', 'uz', 've', 'vi', 'vo', 'wa', 'wo', 'xh', 'yi', 'yo',
			'za', 'zh', 'zu'
		];
		...
		{
			$option->error(\XF::phrase('snog_movies_error_valid_639'));
			return false;
		}

		if ($countryCheck && !in_array(strtoupper($countryCheck), $verifyCountry))
		{
			$option->error(\XF::phrase('snog_movies_error_valid_3166'));
			return false;
		}
