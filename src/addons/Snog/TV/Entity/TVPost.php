 ...
 *
 * RELATIONS
 * @property \Snog\TV\XF\Entity\Post $Post
 */
class TVPost extends Entity
{
	public function getTmdbTvId()
	{
		$post = $this->Post;
	...
	{
		/** @var \Snog\TV\Helper\Tmdb\Episode $tmdbHelper */
		$tmdbHelper = \XF::helper('Snog\TV:Tmdb\Episode');
	...
	protected function _postDelete()
	{
		...
		$imageService = $this->app()->service('Snog\TV:TVPost\Image', $this);
		$imageService->deleteImageFile();

		$db = $this->db();
		$db->delete('xf_snog_tv_post', 'post_id = ?', $this->post_id);
	}
