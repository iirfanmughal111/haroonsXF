<?php
// FROM HASH: 55ebd50d08201a41ca70920153ff0c37
return array(
'code' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Buying history');
	$__finalCompiled .= '
...
<div class="block-container">
	...
	if ($__templater->isTraversable($__vars['results'])) {
		foreach ($__vars['results'] AS $__vars['result']) {
			...
			if (($__vars['xf']['visitor']['user_id'] == $__vars['result']['seller_id']) AND (($__vars['editLimit'] < $__vars['result']['timestamp']) AND (!$__templater->method($__vars['xf']['visitor'], 'hasPermission', array('trader', 'admin', ))))) {
				$__compilerTemp2[] = array(
					'_type' => 'cell',
					'html' => '<a href="' . $__templater->func('link', array('trader/editbuyer', '', array('trader_id' => $__vars['result']['trader_id'], ), ), true) . '">' . 'Edit' . '</a>',
				);
			}
			if (($__vars['xf']['visitor']['user_id'] == $__vars['result']['seller_id']) AND (($__vars['editLimit'] >= $__vars['result']['timestamp']) AND (!$__templater->method($__vars['xf']['visitor'], 'hasPermission', array('trader', 'admin', ))))) {
				$__compilerTemp2[] = array(
			...
			if (($__vars['xf']['visitor']['user_id'] != $__vars['result']['seller_id']) AND (!$__templater->method($__vars['xf']['visitor'], 'hasPermission', array('trader', 'admin', )))) {
				$__compilerTemp2[] = array(
					'_type' => 'cell',
					'html' => '&nbsp',
				);
