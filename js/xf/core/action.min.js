...
$jscomp.findInternal = function (c, g, e) {
  ...
  for (var h = c.length, a = 0; a < h; a++) {
    var b = c[a];
    if (g.call(e, b, a, c)) return { i: a, v: b };
  }
  return { i: -1, v: void 0 };
};
$jscomp.ASSUME_ES5 = !1;
...
!(function (c, g, e, h) {
  XF.AttributionClick = XF.Event.newHandler({
    ...
    click: function (a) {
      var b = this.options.contentSelector,
        d = c(b);
      d.length &&
        (a.preventDefault(), XF.smoothScroll(d, b, XF.config.speed.normal));
    },
  });
  XF.LikeClick = XF.Event.newHandler({
    ...
    handleAjax: function (a) {
      ...
      var f = this.options.likeList
        ? XF.findRelativeIf(this.options.likeList, b)
        : c([]);
      "undefined" !== typeof a.html &&
        f.length &&
        (a.html.content
          ? XF.setupHtmlInsert(a.html, function (a, b) {
    ...
    if (b.switchKey) {
      ...
      if (f) {
        for (
          ...
          if (((f = f.substring(e[0].length)), (g = c.trim(e[3])), g.length))
            switch (e[2]) {
              case "addClass":
                a.addClass(g);
                break;
      ...
      d.isDefaultPrevented() ||
        (a.html && a.html.content && this.options.overlayOnHtml
          ? XF.setupHtmlInsert(a.html, function (a, b) {
              ...
              a = XF.getOverlayHtml({ html: a, title: b.h1 || b.title });
              a.find("form").on(
                "ajax-submit:response",
                XF.proxy(f, "handleOverlayResponse")
              );
  ...
  XF.AlertsList = XF.Element.newHandler({
    ...
    _makeAjaxRequest: function (a, b, d) {
      if (!this.processing) {
        this.processing = !0;
        var c = this;
        XF.ajax("POST", a, d || {}, b, { skipDefaultSuccess: !0 }).always(
          function () {
            setTimeout(function () {
              c.processing = !1;
            }, 250);
    ...
    toggleReadStatus: function (a, b) {
      var d = a.hasClass("is-unread"),
        ...
        e = c.data("content");
      d
        ? (a.removeClass("is-unread"), (e = c.data("unread")))
        : b && (a.addClass("is-unread"), (e = c.data("read")));
  ...
  XF.ShareButtons = XF.Element.newHandler({
    ...
    init: function () {
      ...
      this.$target.find(a).each(function () {
        var a = c(this);
        b &&
          XF.Element.applyHandler(a, "element-tooltip", { element: "> span" });
        a.data("clipboard") &&
          navigator.clipboard &&
    ...
    setupPageData: function () {
      this.options.pageTitle && this.options.pageTitle.length
        ...
        : ((this.pageTitle = c('meta[property="og:title"]').attr("content")),
          this.pageTitle || (this.pageTitle = c("title").text()));
      this.options.pageUrl && this.options.pageUrl.length
        ? (this.pageUrl = this.options.pageUrl)
        : ((this.pageUrl = c('meta[property="og:url"]').attr("content")),
          this.pageUrl || (this.pageUrl = g.location.href));
      this.options.pageDesc && this.options.pageDesc.length
        ? (this.pageDesc = this.options.pageDesc)
        : ((this.pageDesc = c('meta[property="og:description"]').attr(
            "content"
          )),
          this.pageDesc ||
            (this.pageDesc =
      ...
      this.options.pageImage && this.options.pageImage.length
        ...
        : ((this.pageImage = c('meta[property="og:image"]').attr("content")),
          this.pageImage ||
    ...
    setupShareData: function () {
      this.fetchUrl ||
        (this.options.url
          ...
          : ((this.url = c('meta[property="og:url"]').attr("content")),
            this.url || (this.url = g.location.href)),
        this.options.title
          ? (this.title = this.options.title)
          : ((this.title = c('meta[property="og:title"]').attr("content")),
            this.title || (this.title = c("title").text())),
        this.options.text
          ? (this.text = this.options.text)
          : ((this.text = c('meta[property="og:description"]').attr("content")),
            this.text ||
              (this.text = c("meta[name=description]").attr("content") || "")));
    },
    click: function (a) {
      a.preventDefault();
      if (this.fetchUrl) {
  ...
  XF.Reaction = XF.Element.newHandler({
    ...
    init: function () {
      if (this.$target.is("a") && this.$target.attr("href")) {
        ...
        a.length &&
          ((this.$tooltipHtml = c(c.parseHTML(a.html()))),
      ...
      if (a.html) {
        ...
        XF.setupHtmlInsert(a.html, function (a, c, h) {
          ...
          f
            ? (b.addClass("has-reaction"),
              ...
              d && b.removeClass("reaction--" + d),
              b.addClass("reaction--" + f),
              b.data("reaction-id", f))
            : (b.removeClass("has-reaction"),
              b.addClass("reaction--imageHidden"),
              d &&
                (b.removeClass("reaction--" + d),
                b.addClass("reaction--" + a.data("reaction-id")),
        ...
        "undefined" !== typeof a.reactionList &&
          ...
          (a.reactionList.content
            ? XF.setupHtmlInsert(a.reactionList, function (a, b) {
        ...
        XF.ajax(
          ...
          function (a) {
            a.html &&
              (b.$showAllLinkTarget &&
                a.showAllUrl &&
                b.$showAllLinkTarget.attr("href", a.showAllUrl),
              XF.setupHtmlInsert(a.html, function (a, c) {
    ...
    handleAjax: function (a) {
      ...
      if (this.options.contentId) {
        ...
        b.each(function () {
          if (e[0] !== this) {
            var b = c(this);
            b.is('[data-xf-init~="content-vote"]')
              ? XF.Element.getHandler(b, "content-vote").updateData(a)
              : d.updateDisplay(b, a);
          }
        });
      }
