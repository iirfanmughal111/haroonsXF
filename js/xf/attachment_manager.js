!(function ($, window, document, _undefined) {
  ...
  XF.AttachmentManager = XF.Element.newHandler({
    ...
    init: function () {
      ...
      if (android && parseInt(android[1], 10) < 5) {
        ...
        if (!chrome || parseInt(chrome[1], 10) < 33) {
          console.warn(
            "Old Android WebView detected. Must fallback to basic uploader."
          );
          return;
        }
      }
      ...
      if (this.options.manageUrl) {
        ...
        this.manageUrl =
          $uploader.data("upload-href") || $uploader.attr("href");
      }
      ...
      if (this.$container.length) {
        this.$container
          ...
          .on(
            "click",
            options.selectToggleButton,
            XF.proxy(this, "selectToggleClick")
          )
          .on(
            "click",
            options.selectActionButton,
            XF.proxy(this, "selectActionClick")
          );
      } else {
        this.legacyMode = true;

        this.$filesContainer.on(
          "click",
          options.actionButton,
          XF.proxy(this, "actionButtonClick")
        );
      ...
      setTimeout(function () {
        self.editor = XF.getEditorInContainer(
          self.$target,
      ...
      if (this.$form.length) {
        ...
        this.$form.on(
          "attachment-manager:reset",
          XF.proxy(this, "resetAttachments")
        );
      }
    },
    ...
    setupUploadButtons: function ($uploaders, flow) {
      ...
      $uploaders.each(function () {
        var $button = $(this),
          ...
          $target = $('<span class="js-attachButton" />')
            .insertAfter($button)
            .append($button);
    ...
    fileAdded: function (file) {
      ...
      var $hScoller = this.$filesContainer.closest(
      ...
      if (
        ...
        this.uploadError(
          file,
          this.addErrorToJson({}, XF.phrase("file_too_large_to_upload"))
        );
        return false;
      }
      if (
        XF.config.uploadMaxFilesize > 0 &&
        file.size > XF.config.uploadMaxFilesize
      ) {
        this.uploadError(
          file,
          this.addErrorToJson(
    ...
    resizeProgress: function ($row, progress) {
      ...
      $inner.text(percent + "%").css("width", percent + "%");
    },
    ...
    uploadSuccess: function (file, message, chunk) {
      ...
      if (json.attachment) {
        this.insertUploadedRow(
          json.attachment,
          this.fileMap[file.uniqueIdentifier]
        );
      } else {
    ...
    checkboxClick: function () {
      var checkedCount = this.$filesContainer
        .find("input:checkbox")
        .filter(":checked").length;

      // disable action buttons if nothing is selected
      $(this.options.selectActionButton).prop(
        "disabled",
        checkedCount ? false : true
      );
    },
    ...
    selectActionClick: function (e) {
      ...
      var action = $(e.currentTarget).attr("data-action"),
        ...
        $checked = this.$filesContainer.find(
          rowSelector + " " + "input[type=checkbox]:checked"
        );
    ...
    insertAttachment: function ($row, action, type) {
      ...
      if (action == "full") {
        ...
        if (type == "image") {
          ...
          html =
            '<span contenteditable="false" draggable="true" class="fr-audio fr-dvi fr-draggable fr-deletable"><audio data-attachment="full:{{id}}" src="{{img}}" controls></audio></span>';
        }
    ...
    removeInsertButtons: function ($container) {
      $container
        .find(this.options.insertRow + "," + this.options.insertMultiRow)
        .remove();
    ...
    toggleInsertMultiRow: function () {
      ...
      var $rows = this.$filesContainer
          .find(this.options.actionButton)
          .filter(":not([data-action=delete])")
          .closest(this.options.fileRow),
  ...
  XF.AttachmentOnInsert = XF.Element.newHandler({
    ...
    onAttachmentInsert: function (e, $html, manager) {
      ...
      XF.ajax("post", href, data, XF.proxy(this, "onLoad")).always(function () {
        self.loading = false;
      });
    },
    ...
    onLoad: function (data) {
      ...
      XF.setupHtmlInsert(data.html, function ($html, container, onComplete) {
        self.$target
          .replaceWith($html)
          .xfFadeDown(XF.config.speed.xfast, function () {
            onComplete(true);
            XF.layoutChange();
          });
      });
